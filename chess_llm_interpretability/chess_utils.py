...
def board_to_legal_moves_state(
    board: chess.Board, skill: Optional[int] = None
) -> torch.Tensor:
...
def board_to_last_self_move_state(
    board: chess.Board, skill: Optional[int] = None
) -> torch.Tensor:
    """Given a chess board object, return an 8x8 torch.Tensor.
...
def create_state_stack(
    ...
    initial_states_lRR.append(
        custom_board_to_state_fn(board, skill).to(dtype=torch.int8)
    )
    ...
    for char in moves_string:
        if char == " ":
            move_index += 1
        expanded_states_lRR.append(
            initial_states_lRR[min(move_index, len(initial_states_lRR) - 1)]
        )
...
def create_state_stacks(
    ...
    for idx, pgn_string in enumerate(moves_strings):
        ...
        state_stack_lRR = create_state_stack(
            pgn_string, custom_board_to_state_fn, skill
        )
    ...
    final_state_stack_MBlRR = final_state_stack_BlRR.unsqueeze(
        0
    )  # Add a dimension for the modes
...
def find_custom_indices(
    custom_indexing_fn: Callable, games_strs_Bl: list
) -> torch.Tensor:
...
def update_config_using_player_color(
    ...
    if player_color == PlayerColor.BLACK:
        config.linear_probe_name = config.linear_probe_name.replace(
            "probe", "black_player_probe"
        )
